...
function MOI.jacobian_structure(md::MDProblem)
    n, k, m = size(md)
    return rowcol_of_dense(n+k, m+1)
end
...
function MOI.eval_constraint_jacobian(md::MDProblem, J, u)
    ...
    ∂g = Dg(θ, π, md)
    @. ∂g = ∂g
    return assign_constraint_jacobian!(J, G, ∂g)
end
...
function MOI.eval_objective(md::SMDProblem, u::Vector{Float64})
    return divergence(md)(u)
end

function MOI.eval_objective_gradient(md::SMDProblem, res, u)
    n, k, m = size(md)
    T = eltype(res)
    return Divergences.gradient!(res, divergence(md), u)
end
...
function MOI.eval_constraint_jacobian(md::MDProblem, J, u)
    ...
    G = md.G
    #@. G = G/n
    return assign_constraint_jacobian!(J, G)
end
...
function lagrangian(md::MDProblem, u, σ, λ)
    ...
    π = u[1:n]
    θ = u[(n + 1):(n + k)]
    return σ .* divergence(md)(π) + mean(π .* g(θ, md)*λ)
end
