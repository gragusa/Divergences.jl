...
- ReverseKullbackLeibler γ at a = 0 — `loga` now folds a = 0 into the +∞ boundary branch so `log` is only ever called on a > 0, making the limit robust regardless of whether the `log` in scope returns -Inf, NaN, or throws at 0
